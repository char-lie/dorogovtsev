...
\end{comment}
Уменьшим размеры полуинтервалов $I_k$
\begin{equation*}
...
\begin{proof}
  ...
  \end{equation}
  Мы знаем, что в пространстве интегрируемых с квадратом случайных величин
  ...
  \begin{equation}\label{eq:collinearityRandomVariable}
...
\footnotetext{Подробнее с дифференцированием под знаком
...
\begin{example}
  Пусть \xsample --- выборка из распределения $N\left( 0, 1 \right)$.
  Докажем, что эффективная оценка параметра $\theta$ имеет вид
  \begin{equation*}
    \hat{\theta} = \overline{x} = \frac{1}{n} \cdot \sum_{k=1}^{n} x_k
  \end{equation*}

  Для начала найдём её дисперсию
  \begin{equation*}
    \dispersionof{\theta}{\hat{\theta}}
    = \dispersionof{\theta}{\overline{x}}
    = \frac{1}{n^2} \cdot \sum_{k=1}^{n} \dispersionof{\theta}{x_k}
    = \frac{1}{n} \cdot \dispersionof{\theta}{x_1}
    = \frac{1}{n}
  \end{equation*}
  Теперь посмотрим на логарифм функции правдоподобия
  \begin{equation*}
    \begin{split}
      \ln{L\left( \vec{x}, \theta \right)}
      &= \ln{\left\{ \left( \frac{1}{\sqrt{2 \cdot \pi}} \right)^n
        \cdot \exp{\left( - \frac{1}{2} \cdot \sum_{k=1}^{n}
          \left( x_k - \theta \right)^2 \right)}\right\}} \\
      &= -n \cdot \ln{\sqrt{2 \cdot \pi}} - \frac{1}{2}
        \cdot \sum_{k=1}^{n} \left( x_k - \theta \right)^2
    \end{split}
  \end{equation*}
  Не составит труда найти вклад выборки
  \begin{equation*}
    U\left( \vec{x}, \theta \right)
    = \frac{\partial}{\partial\theta} \ln{L\left( \vec{x}, \theta \right)}
    = \sum_{k=1}^{n}\left( x_k - \theta \right)
  \end{equation*}
  Посчитаем количество информации Фишера
  \begin{equation*}
    I_n\left( \theta \right)
    = \Meanof{\theta}{\sum_{k=1}^{n} \left( x_k - \theta \right)}^2
    = \Dispersionof{\theta}{\sum_{k=1}^{n} \left( x_k - \theta \right)}
    = n \cdot \dispersionof{\theta}{x_1}
    = n
  \end{equation*}
  Видим, что условие
  $\dispersionof{\theta}{\hat{\theta}} = \frac{1}{I_n\left( \theta \right)}$
  выполняется, а это значит, что оценка $\hat{\theta}$ является эффективной
\end{example}
...
\end{comment}
В силу \eqref{eq:collinearityRandomVariable} последнее равенство влечёт за
собой
\begin{equation*}
  \frac{U\left( \vec{x}, \theta \right)}{
    \sqrt{\meanof{\theta}{U\left( \vec{x}, \theta \right)^2}}}
    = \frac{\hat{\theta} - \theta}{\Meanof{\theta}{\hat{\theta} - \theta}^2}
\end{equation*}
Распишем как
\begin{equation*}
  U\left( \vec{x},\theta \right)
...
\begin{equation*}
  a\left( \theta \right)
    = \integralp{}{}{\theta}{k\left( \theta \right)},
  \qquad b\left( \theta \right)
    = - \integralp{}{}{\theta}{\theta \cdot k\left( \theta \right)}
\end{equation*}
Получаем
\begin{equation*}
...
Предположим, что существует эффективная оценка $\theta$ для $n=1$.
Тогда в силу доказанного
\begin{equation*}
  \pdf{x,\theta}
  = \exp{\left\{\hat{\theta}\left( x \right) \cdot a\left( \theta \right)
    + b\left( \theta \right) + c\left( x \right)\right\}}
\end{equation*}
При этом для $n>1$ получим
\begin{equation*}
  L\left( \vec{x},\theta \right)
  = \prod_{k=1}^n\pdf{x_k,\theta}
  = \exp{\left\{
    \sum_{k=1}^n \hat{\theta}\left(x_k\right) \cdot a\left(\theta\right)
    + n \cdot b\left( \theta \right)
    + \sum_{k=1}^n c\left( x_k \right)\right\}}
\end{equation*}
Тогда для несмещённой оценки
$\hat{\theta}_n\left( \vec{x} \right)
= \frac{1}{n} \cdot \sum_{k=1}^{n} \hat{\theta}\left( x_k \right)$
с помощью дифференцирования получим равенство
\begin{equation*}
  U\left( \vec{x}, \theta \right)
  = \left( \hat{\theta}_n - \theta \right) \cdot n \cdot k\left( \theta \right)
\end{equation*}
Значит, оценка $\hat{\theta}_n$ действительно эффективна.
...
\begin{affirmation}
  ...
  \index{экспоненциальное!распределение!эффективная оценка}
  \index{распределение!экспоненциальное!эффективная оценка}
  Если существует эффективная оценка $\hat{\theta}$ параметра $\theta$ при
  $n=1$, то распределение экспоненциально, и существует эффективная оценка
  $\theta$ для любого $n$
  \begin{equation*}
    \hat{\theta}_n\left( \vec{x} \right)
    = \frac{1}{n} \cdot \sum_{k=1}^{n} \hat{\theta}\left( x_k \right)
...
Попробуем предложить рецепт нахождения эффективной оценки. Начнём с примера
\begin{example}
  ...
  Когда ззначение параметра $\theta$ равно эффективной оценке $\overline{x}$,
  функция правдоподобия принимает максимальное значение.
...
\begin{remark}
  Оценка максимального правдоподобия не обязана быть даже несмещённой --- не
  только эффективной.
  В рассмотренном примере для $n=1$ вообщще не существует
  $\mean{\lambda_*} = \mean{\frac{1}{x_1}}$
\end{remark}
...
\begin{example}
  Пусть \xsample --- выборка из равномерного распределения на
  $\left[ 0; \theta \right],\; \theta > 0$.
  ...
  Обозначим
  \begin{equation*}
    \alpha = \probability{\theta' < \theta - \delta}
  \end{equation*}
  Поскольку $\theta_* \le \theta'$, имеем
  \begin{equation*}
    \probability{\theta_* < \theta - \delta}
    \ge \probability{\theta' < \theta - \delta}
    = \alpha
  \end{equation*}
  С другой стороны
  \begin{equation*}
    \probability{\theta_* < \theta - \delta}
    = \probability{x_1 < \theta - \delta, \dots, x_n < \theta - \delta}
