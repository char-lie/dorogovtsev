...
\begin{example}
    ...
    В таком случае поверхностями уровня $S_t$ будут гиперплоскости
    $$S_t = \left\{ \vec{u} \mid \left( \vec{u}, \vec{e} \right) = t \right\}$$

    Нормалью к гиперплоскостям будет вектор $\vec{e}$.
    Гиперплоскость $S_t$ проходит через точку с радиус-вектором
    $t \cdot \vec{e}$.\footnote{По определению уравнение гиперплоскости
    с нормалью $\vec{e}$, проходящую через точку с радиус-вектором $\vec{x}$,
...
Чтобы свести задачу к уже известной, нужно найти совместную плотность
случайных величин $f\left( \vec{x} \right)$ и $g\left( \vec{x} \right)$,
поэтому мы пойдём другим --- более творческим --- путём.

Нетрудно догадаться, что для того, чтобы найти ``вес'' поверхности уровня,
нужно взять поверхностный интеграл плотности. Но не всё так просто.

Чтобы мы не получали нулевую вероятность события
$\left\{ g\left( \vec{x} \right) = t \right\}$,
будем считать объём её раздутия.
Поместим поверхность в своеобразный кокон,
ширина которого в каждой точке будет тем ближе к поверхности,
...
находится очень близко к $t$, будет приблизительно равна
плотности распределения $g\left( \vec{x} \right)$ в этой точке
(если таковая имеется), умноженной на ширину промежутка --- погрешность,
...
Вернёмся к раздутию. Если обозначить через $\varepsilon$ расстояние от
точки $\vec{u}_0$ до ближайшей к нему точки кокона, то, вспомнив то условие,
что расстояние должно быть обратно пропорционально стремительности изменения
уровней в этой окрестности.
Под описание такой величины прекрасно подходит модуль градиента.
Поскольку значение $g\left( \vec{x} \right)$ не меняется вдоль поверхности
...
Нас интересует прирост координат $\varepsilon$ в точке $u_0$,
