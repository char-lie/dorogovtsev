...
Снова воспользуемся линейностью математического ожидания
$$\sum_{k=1}^n \mean{\left( \eta \cdot \indicatorof{H_k} \right)}
    \cdot \probability{A \mid H_k}
    = \mean{\left( \sum_{k=1}^n
        \eta \cdot \indicatorof{H_k} \cdot \indicator{H_k \subseteq A}\right)}$$
...
то напомню о морально-этической стороне дела: сумма нужна лишь для того,
чтобы пройтись по всем возможным индикатором, из которых лишь один сработает,
...
$$\mean{\left( \sum_{k=1}^n
    \eta \cdot \indicatorof{H_k} \cdot \indicator{H_k \subseteq A}\right)}
    = \mean{\left( \eta \left( \omega \right) \cdot \sum_{k=1}^n
        \indicatorof{H_k}\left( \omega \right)
            \cdot \indicator{H_k \subseteq A}\right)}$$

Очевидно, что в сумме останутся лишь те индикаторы,
уровни которых включены в $A$
$$\mean{\left( \eta \left( \omega \right) \cdot \sum_{k=1}^n
    \indicatorof{H_k}\left( \omega \right)
        \cdot \indicator{H_k \subseteq A}\right)}
...
Видимо, что равенство \eqref{scalarMulEquality} доказано.
...
\begin{definition}[Условное математическое ожидание случайной величины
    ...
    \index{условное математическое ожидание}
    Условным математическим ожиданием $\eta$
