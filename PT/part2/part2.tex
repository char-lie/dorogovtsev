...
\begin{proof}
    ...
    Допустим, есть такая непустая подпсовокупность $B_1, \dots, B_m$ и
    такой набор $\varepsilon_1', \dots, \varepsilon_m'=\pm 1$, для
    которых не выполняется условие независимости в совокупности
    $$\probability{\bigcap_{k=1}^m B_{k}^{\varepsilon_k'}}
    =\prod_{k=1}^m \probability{B_{k}^{\varepsilon_k'}}$$

    Обозначим множество событий $A_k$ через $A$,
    а множество событий $B_k$ через $B$
    \begin{align*}
        A&=\left\{A_1, \dots, A_n\right\}\\
        B&=\left\{B_1, \dots, B_m\right\}
    \end{align*}

    Тогда существует разность $C=A\setminus B=\left\{C_1, \dots, C_l\right\}$
    и для любого набора $\eta_1, \dots, \eta_l=\pm 1$ выполняется
    $$
    \probability{
        \bigcap_{k=1}^m B_{k}^{\varepsilon_k'}
        \cap \bigcap_{k=1}^l C_{k}^{\eta_k}}
    =\prod_{k=1}^m \probability{B_{k}^{\varepsilon_k'}}
        \cdot \prod_{k=1}^l \probability{C_{k}^{\eta_k}}$$

    Пересечение всех событий из множества $B$
    со всеми элементами из множества $C$
    даёт пересечение всех элементов из множества $A$,
    таким образом получаем тождество, написанное в самом начале доказательства.
